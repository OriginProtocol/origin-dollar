...
const Collateral = () => {
  ...
  const chartData = collateral.collateral?.map((token) => {
    return (
      { title: token.name.toUpperCase(), value: (token.total / total) * 100, color: colors[token.name] || '#ff0000'}
    )
  })
      ...
      <section className='home dim m-0'>
        <div className='pb-20 px-8 text-center'>
          ...
          <Typography.Body2 className='opacity-75'>
            OUSD is backed 1:1 by the most trusted collateral in crypto. Reserves are verifiable on-chain. You can redeem OUSD <br className="hidden md:block" />immediately at any time.
          </Typography.Body2>
          <div className='collateral flex flex-col md:flex-row justify-between rounded-xl my-10 md:m-16 px-6 pb-6 md:p-6 md:px-28 md:py-20'>
            ...
            <div className='md:w-1/2 md:px-20 text-left'>
              <Typography.Body className='mb-3'>Currently-held collateral</Typography.Body>
                ...
                {collateral.collateral?.map((token) => {
                  return (
                    ...
                    >
                      ...
                      <div>
                        <Typography.H6>{`${formatCurrency((token.total / total) * 100, 2)}%`}</Typography.H6>
                        <div className='flex flex-row pt-2'>
                          <Typography.Body2 className='opacity-75 pr-2'>{`$${formatCurrency(token.total, 0)}`}</Typography.Body2>
                          {/*<a
