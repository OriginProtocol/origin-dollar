...
const ApproveCurrencyInProgressModal = ({ show }) => {
  ...
  const approveContractTypes = Object.keys(currencies).map(c => `approve-${c}`)
  const pendingApprovalTransactions = transactions.filter(tx => approveContractTypes.includes(tx.type) && !tx.mined)
  const pendingApprovalCoins = pendingApprovalTransactions.map(pat => pat.type.substr(pat.type.indexOf('-') + 1))
    ...
    >
      ...
      >
        <div className="body-coins d-flex flex-column">
          ...
          <h2>{fbt('Waiting for ' + fbt.param('coins-name', pendingApprovalCoins.join(', ').toUpperCase()) + ' to be approved', 'Waiting for coin to be approved')}</h2>
        </div>
      </div>
    </div>
    <style jsx>{`
