  		...
  		<div className={`blue-chin d-flex flex-column ${semiExtend && !fullExtend ? 'semi-extended' : ''} ${fullExtend ? 'extended' : ''}`}>
  			<div className="main-body d-flex flex-column justify-content-between">
  				...
  				{semiExtend && <div className={`main-body-footer flex-grow-1 d-flex align-items-center justify-content-center ${displayFooterContentsBorder ? 'boredered' : ''}`}>
  					{displayFooterContents && fbt('When you unstake, your OGN is claimed automatically', 'Unstake information message')}
  				</div>}
  			</div>
				{displayChinContents && <div className={`chin-contents d-flex ${showChinContents ? 'visible' : ''}`}>
					<div className="balance-box d-flex flex-column">
						<div className="title">
					...
					<div className="weekly-rate d-flex align-items-center justify-content-center ml-auto">
						<img className="ogn-icon" src="/images/ogn-icon-white-border.svg"/>
						{fbt('Your rate: ' + fbt.param('weekly-rate', pool.your_weekly_rate) + ' OGN/week', "user's weekly rate")}
					</div>
