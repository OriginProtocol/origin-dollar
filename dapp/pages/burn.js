...
const renderer = ({ days, hours, minutes, seconds, completed }) => {
  if (completed) {
    ...
    return <BurnCountdown days={days} hours={hours} minutes={minutes} seconds={seconds} />
  }
}
...
export default function Burn({ locale, onLocale }) {
  ...
  const mandatoryLockupDistributor = '0xD667091c2d1DCc8620f4eaEA254CdFB0a176718D';
  ...
  useEffect(() => {
    ...
    const fetchStakedOgv = async () => {
      const staked = await ogv.balanceOf(addresses.mainnet.veOGV).then((r) => Number(r) / 10 ** 18)
      const supply = await ogv.totalSupply().then((r) => Number(r) / 10 ** 18)
      const optional = await ogv.balanceOf(optionalLockupDistributor).then((r) => Number(r) / 10 ** 18)
      const mandatory = await ogv.balanceOf(mandatoryLockupDistributor).then((r) => Number(r) / 10 ** 18)
      const totalVe = await veogv.totalSupply().then((r) => Number(r) / 10 ** 18)
      ...
      <section className="burn black">
        <div className="container d-flex flex-column text-align-left ml-lg-5 pl-lg-5">
          ...
          <div className='text-container mt-5'>
            ...
            {' '}
            <span className='subtext'>{fbt('(currently unclaimed OGV)', '(currently unclaimed OGV')}</span>
          </div>
          ...
          <h3>
            <span className='percent gradient1'>{`${formatCurrency(burnAmount / initialSupply * 100, 2)}% `}</span>
            {fbt('of initial supply', (' of initial supply'))}
          </h3>
          <div className='links flex-row'>
            <div className='link'>
            <a
              href={'https://etherscan.io/address/0x7ae2334f12a449895ad21d4c255d9de194fe986f'}
              target='_blank'
              rel='noopener noreferrer'
            >
            ...
            <div className='link'>
            <a
              href={'https://etherscan.io/address/0xd667091c2d1dcc8620f4eaea254cdfb0a176718d'}
              target='_blank'
              rel='noopener noreferrer'
            >
          ...
          <div className='stats d-flex flex-column'>
            <div className='d-flex flex-row'>
              <div className='info-box airdrop'>
                ...
                <div className='mb-4'>
                  <span className='large'>{formatCurrency(airdropAllocation, 0)}</span>
                  <span>{' OGV'}</span>
                </div>
                <div className='d-flex flex-row'>
                  <div className='mr-lg-5 mr-md-3'>
                    ...
                    <span className='medium'>{formatCurrency(airdropAllocationOgn, 0)}</span>
                    <span>{' OGV'}</span>
                  ...
                  <div>
                    ...
                    <span className='medium'>{formatCurrency(airdropAllocationOusd, 0)}</span>
                    <span>{' OGV'}</span>
              ...
              <div className='medium mb-3'>Claim stats</div>
                ...
                <div className='mb-4'>
                  <span className='large'>{formatCurrency(airdropAllocationOgn - burnAmount, 0)}</span>
                  <span>{' OGV'}</span>
                  <span className='grey'>{` (${formatCurrency((airdropAllocation - burnAmount) * 100 / airdropAllocation, 2)}%)`}</span>
                </div>
                <div className='d-flex flex-row'>
                  <div className='mr-lg-5 mr-md-3'>
                    ...
                    <span className='medium'>{formatCurrency(airdropAllocationOgn - optionalLockupBalance, 0)}</span>
                    <span>{' OGV'}</span>
                    <div className='grey'>{`(${formatCurrency((airdropAllocationOgn - optionalLockupBalance) / airdropAllocationOgn * 100, 2)}%)`}</div>
                  </div>
                  <div>
                    <div className='text-container grey'>
                      OUSD holders
                    </div>
                    <span className='medium'>{formatCurrency(airdropAllocationOusd - mandatoryLockupBalance, 0)}</span>
                    <span>{' OGV'}</span>
                    <div className='grey'>{`(${formatCurrency((airdropAllocationOusd - mandatoryLockupBalance) / airdropAllocationOusd * 100, 2)}%)`}</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className='d-flex flex-row'>
                  <div className='mr-lg-5 mr-md-4'>
                    ...
                    <span className='large'>{formatCurrency(totalStaked, 0)}</span>
                    <span>{' OGV'}</span>
                  </div>
                  <div className='ml-lg-3'>
                    ...
                    <span className='large'>{`${formatCurrency(totalStaked / totalSupply * 100, 2)}%`}</span>
                  </div>
                </div>
              </div>
              <div className='apy d-flex flex-column text-center'>
                <div>
                  <span className='large'>{`${formatCurrency(stakingApy, 2)}%`}</span>
                  {' APY'}
                </div>
                <div>
                  <a
