...
contract Base is PTest {
    ...
    function getWETH(address to, uint amount) public {
        vm.startPrank(to);
        vm.deal(to, amount);
        WETH.call{value: amount}("");
        vm.stopPrank();

        require(IERC20(WETH).balanceOf(to) >= amount, "incorrect wrapped ETH");
    }

    function getDAI(address from, address to, uint daiAmount) public {
        ...
        IUNISWAP_V2_ROUTER(UNI_ROUTER).swapTokensForExactTokens(
            daiAmount, 
            type(uint256).max,
            path, 
            to, 
        ...
        require(IERC20(DAI).balanceOf(to) >= daiAmount, 
            "incorrect swapped dai");
    }

    function getUSDT(address from, address to, uint usdtAmount) public {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = USDT;
        ...
        require(IERC20(USDT).balanceOf(to) >= usdtAmount, 
            "incorrect swapped usdt");
    }

    function getUSDC(address from, address to, uint usdcAmount) public {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = USDC;
        ...
        require(IERC20(USDC).balanceOf(to) >= usdcAmount, 
            "incorrect swapped usdc");
    }
}