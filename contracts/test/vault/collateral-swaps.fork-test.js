  ...
  describe("OETH Vault", () => {
    ...
    describe("Collateral swaps", async () => {
      ...
      for (const test of tests) {
        it(`should be able to swap ${test.fromAmount} ${
          test.from
        } for a min of ${test.minToAssetAmount} ${test.to} using ${
          test.protocols || "all"
        } protocols`, async () => {
    ...
    describe("post deployment", () => {
      ...
      it("assets should have allowed slippage", async () => {
        ...
        for (let i = 0; i < assets.length; i++) {
          ...
          expect(config.decimals, `decimals ${i}`).to.equal(
            expectedDecimals[i]
          );
          expect(config.isSupported, `isSupported ${i}`).to.be.true;
          expect(config.unitConversion, `unitConversion ${i}`).to.be.equal(
    ...
    describe("Collateral swaps", async () => {
      ...
      const tests = [
        ...
        },
        {
          ...
          fromAmount: 1000000,
          minToAssetAmount: 999000,
          slippage: 0.7, // Max 1Inch slippage
        },
        {
          from: "USDC",
          to: "DAI",
