...
forkDescribe('Vault', function () {
  ...
  describe('Admin', () => {
    it('Should have the correct governor address set', async () => {
      const { vault } = fixture
      expect(await vault.governor()).to.equal('0x72426BA137DEC62657306b12B1E869d43FeC6eC7')
    })

    it('Should have the correct strategist address set', async () => {
      const { vault } = fixture
      expect(await vault.strategistAddr()).to.equal('0xF14BBdf064E3F67f51cd9BD646aE3716aD938FDC')
    })
    ...
    it('Should have the correct turstee address set', async () => {
      ...
      expect(await vault.strategistAddr()).to.equal('0x77314eb392b2be47c014cde0706908b3307ad6a9')
    })
  ...
  describe('Capital', () => {
    ...
    it('Should allow to mint and redeem', async () => {
      ...
      const balancePreMint = await ousd.connect(josh).balanceOf(josh.getAddress())
      await vault.connect(josh).mint(
        usdt.address,
        usdtUnits('50000'),
        0
      )

      const balancePostMint = await ousd.connect(josh).balanceOf(josh.getAddress())
      expect(balancePostMint.sub(balancePreMint)).to.approxEqual(ousdUnits("50000"))
      
      await vault.connect(josh).redeem(
        ousdUnits('50000'),
        0
      )

      const balancePostRedeem = await ousd.connect(josh).balanceOf(josh.getAddress())
  ...
  describe('Oracle', () => {
    it('Should have correct Price Oracle address set', async () => {
      const { vault } = fixture
      expect(await vault.priceProvider()).to.equal('0x7533365d1b0D95380bc4e94D0bdEF5173E43f954')
    })
  ...
  describe('Assets & Strategies', () => {
    ...
    it('Should NOT have any unknown strategies', async () => {
      ...
      for (const s of strategies) {
        expect(knownStrategies).to.include(s, `Unknown strategy with address: ${s}`)
      }

      for (const s of knownStrategies) {
        expect(strategies).to.include(s, `Known strategy missing from contract: ${s}`)
      }
    })
    ...
    it('Should have correct default strategy set for USDT', async () => {
      ...
      expect(await vault.assetDefaultStrategies(usdt.address)).to.equal('0x5e3646A1Db86993f73E6b74A57D8640B69F7e259')
    })

    it('Should have correct default strategy set for USDC', async () => {
      const { vault, usdc } = fixture

      expect(await vault.assetDefaultStrategies(usdc.address)).to.equal('0x5e3646A1Db86993f73E6b74A57D8640B69F7e259')
    })

    it('Should have correct default strategy set for DAI', async () => {
      const { vault, dai } = fixture

      expect(await vault.assetDefaultStrategies(dai.address)).to.equal('0x5e3646A1Db86993f73E6b74A57D8640B69F7e259')
    })
