const { defaultFixture, oethDefaultFixtureSetup, oeth1InchSwapperFixtureSetup, impersonateAndFundContract } = require("../_fixture")
...
describe("OETH Vault - Swapper", () => {
  ...
  describe("Swap Collateral", () => {
    ...
    it("Should allow Governor to set slippage for assets", async () => {
      ...
      await expect(tx)
        .to
        .emit(oethVault, "SwapSlippageChanged")
    ...
    it("Should allow to change Swapper address", async () => {
      ...
      await expect(tx).to.emit(oethVault, "SwapperChanged").withArgs(weth.address)
    })
    ...
    it("Should allow to swap tokens", async () => {
      const { weth, reth, stETH, frxETH, oethVault, strategist, mockSwapper } = fixture
      ...
      for (const asset of [weth, reth, stETH, frxETH]) {
        ...
        // Call swap method
        const tx = oethVault.connect(strategist).swapCollateral(
          asset.address,
          toAsset.address,
          fromAmount,
          toAmount,
    ...
    it("Should revert swap if received less tokens than strategist desired", async () => {
      ...
      await mockSwapper.connect(strategist).setNextOutAmount(utils.parseEther("98"))
    ...
    it("Should revert swap if received less tokens than Oracle slippage", async () => {
      ...
      // Mock to return lower than slippage next time
      await mockSwapper.connect(strategist).setNextOutAmount(utils.parseEther("50"))
    ...
    it("Should revert if not called by Governor or Strategist", async () => {
      ...
      // Call swap method
      const tx = oethVault.connect(josh).swapCollateral(
        weth.address,
        stETH.address,
        fromAmount,
        toAmount,
  ...
  describe("1inch Swapper", () => {
    ...
    it("Should swap assets using 1inch router", async () => {
      const { swapper1Inch, strategist, weth, frxETH, mock1InchSwapRouter } = fixture
      ...
      await weth.connect(strategist).mintTo(swapper1Inch.address, fromAmount.mul(2))
      await frxETH.connect(strategist).mintTo(swapper1Inch.address, toAmount.mul(2))

      const tx = swapper1Inch.connect(strategist).swap(
        weth.address,
        frxETH.address,
        fromAmount,
        toAmount,
      ...
      expect(r.logs[0].data).to.equal("")
    })
