...
const shouldHaveRewardTokensConfigured = (context) => {
  ...
  describe("Reward Tokens", () => {
    it("Should have swap config for all reward tokens from strategies", async () => {
      ...
      expectedConfigs = Object.keys(expectedConfigs).reduce((o, k) => ({
        ...o,
      ...
      for (const strategyAddr of strategies) {
        ...
        if (rewardTokens.length) {
          ...
          expect(
            await harvester.supportedStrategies(strategy.address)
          ).to.be.true
          ...
          for (let token of rewardTokens) {
            ...
            expect(config.harvestRewardBps).to.eq(expectedConfig.harvestRewardBps)
            expect(config.allowedSlippageBps).to.eq(expectedConfig.allowedSlippageBps)
            expect(config.liquidationLimit).to.eq(expectedConfig.liquidationLimit)

            if (config.platform == 0) {
              // Uniswap V2
              expect(await harvester.uniswapV2Path(token))
                .to.eq(expectedConfig.uniswapV2Path)
            } else if (config.platform == 1) {
              // Uniswap V3
              expect(await harvester.uniswapV3Path(token))
              .to.eq(expectedConfig.uniswapV3Path)
            } else if (config.platform == 2) {
              // Balancer
              expect(await harvester.balancerPoolId(token))
              .to.eq(expectedConfig.balancerPoolId)
            } else if (config.platform == 3) {
              // Curve
              expect((await harvester.curvePoolData(token)).map(x => parseInt(x.toString())))
      ...
      const missingTokenConfigs = Object.keys(expectedConfigs).filter(k => !checkedConfigs.includes(k))
