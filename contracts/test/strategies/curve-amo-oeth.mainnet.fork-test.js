...
describe("Curve AMO OETH strategy", function () {
  ...
  describe("Harvester pause and CoW Harvester", () => {
    ...
    it("Should have CoW Harvester set as harvesterAddress", async () => {
      expect(await curveAMOStrategy.harvesterAddress()).to.equal(
    ...
    it("Should pause and stop reward transfers, then unpause to resume", async () => {
      // Seed strategy with CRV
      await setERC20TokenBalance(curveAMOStrategy.address, crv, "1000", hre);

      const cowHarvesterAddress = addresses.mainnet.CoWHarvester;
      const cowBalBefore = await crv.balanceOf(cowHarvesterAddress);

      // Strategist pauses
      await expect(
        curveAMOStrategy.connect(impersonatedStrategist).pauseHarvester()
      )
        .to.emit(curveAMOStrategy, "HarvesterPaused")
        .withArgs(impersonatedStrategist.address);
      expect(await curveAMOStrategy.harvesterPaused()).to.be.true;

      // collectRewardTokens succeeds but rewards stay in strategy
      await curveAMOStrategy
        .connect(impersonatedStrategist)
        .collectRewardTokens();
      expect(await crv.balanceOf(cowHarvesterAddress)).to.equal(cowBalBefore);
      expect(await crv.balanceOf(curveAMOStrategy.address)).to.be.gt(0);

      // Governor unpauses
      await expect(
        curveAMOStrategy.connect(impersonatedAMOGovernor).unpauseHarvester()
      )
        .to.emit(curveAMOStrategy, "HarvesterUnpaused")
        .withArgs(impersonatedAMOGovernor.address);
      expect(await curveAMOStrategy.harvesterPaused()).to.be.false;

      // Next collect sends CRV to CoW Harvester
      await curveAMOStrategy
        .connect(impersonatedStrategist)
        .collectRewardTokens();
      expect(await crv.balanceOf(cowHarvesterAddress)).to.be.gt(cowBalBefore);
    });

    it("Strategist cannot unpause", async () => {
      await curveAMOStrategy.connect(impersonatedStrategist).pauseHarvester();
      await expect(
        curveAMOStrategy.connect(impersonatedStrategist).unpauseHarvester()
