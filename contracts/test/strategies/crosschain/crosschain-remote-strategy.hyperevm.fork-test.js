...
const { crossChainHyperEVMFixture } = require("../../_fixture-hyperevm");
const {
...
const loadFixture = createFixtureLoader(crossChainHyperEVMFixture);

describe("ForkTest: CrossChainRemoteStrategy (HyperEVM)", function () {
  this.timeout(0);
  ...
  const verifyBalanceCheckMessage = (
    ...
    expect(sourceDomain).to.eq(19); // HyperEVM CCTP domain
    expect(desinationDomain).to.eq(0); // Ethereum CCTP domain
  ...
  it("Should handle deposits", async function () {
    ...
    await replaceMessageTransmitter(19); // HyperEVM CCTP domain
