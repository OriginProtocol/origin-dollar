...
contract("UniswapStrategy", (accounts) => {
    ...
    const check_balances = async (addr) => {
        return Promise.all([
            usdc.methods.balanceOf(addr).call(),
            usdt.methods.balanceOf(addr).call(),
            dai.methods.balanceOf(addr).call(),
        ]);
    };
    ...
    it("initializes, funds, and runs vault depositing ", async () => {
        ...
        // dai/usdt pair
        await vault_instance.supportAsset(PAIRS[0][0]);
        await vault_instance.addStrategy(
            uniswap_instance.address,
            utils.parseUnits("2", 18)
        );
        // await vault_instance.allocate({ from: accounts[0] });
