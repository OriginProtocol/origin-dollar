...
const { loadFixture, units, ousdUnits, expectApproxSupply } = require("../helpers");

describe("Uniswap V3 Strategy", function () {
  this.timeout(0);

  let fixture
  let vault, harvester, ousd, usdc, usdt, dai, cusdc, cusdt, cdai
  ...
  beforeEach(async () => {
    ...
    reserveStrategy = fixture.compoundStrategy
  ...
  describe.only("Mint", function () {
    it("Should deposit to reserve strategy", async () => {
      ...
      console.log((await reserveStrategy.checkBalance(cusdc.address)).toString());
      // await expect(reserveStrategy).has.an.approxBalanceOf("10200", );
    });
  })
