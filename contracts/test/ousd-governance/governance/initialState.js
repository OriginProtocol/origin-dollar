...
describe('Contract: Governance', async () => {
  ...
  describe('Initial state', async () => {
      ...
      it('counting mode should be bravo', async () => {
        expect(await governance.COUNTING_MODE()).to.be.eq('support=bravo&quorum=bravo')
      }),
      ...
      it('quorum should be > 0', async () => {
        expect(await governance.quorum(await getBlockNumber() - 1)).
            to.be.gt(0)
      }),
      
      it('voting power of admin should be 0', async () => {
        expect(await governance.getVotes(admin.address, await getBlockNumber() - 1)).to.be.eq(0)
      }),
