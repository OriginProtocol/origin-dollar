...
describe("Contract: OptionalLockupDistributor", async () => {
  ...
  describe("Claiming", async () => {
    it("should receive tokens on claim with no lockup", async () => {
      ...
      expect(await ogv.balanceOf(claimer.address)).to.be.eq(
        before_balance.add(amount)
      );
    }),
      it("should reflect balance on claim with lockup", async () => {
        let amount = bnDecimal(500000000);
        // Claim and lock for a week
        await optionalLockup
          .connect(claimer)
          .claim(1, amount, merkleProof, week);
      ...
      it("should't be able to claim if claim period has expired", async () => {
        ...
        await expect(
          optionalLockup.connect(claimer).claim(1, amount, merkleProof, 0)
      ...
      it("should be able to burn remaining ogv after claim period", async () => {
        ...
        expect(await ogv.balanceOf(optionalLockup.address)).to.be.eq(0);
      }),
      it("should't be able to burn remaining ogv before claim period", async () => {
        ...
        await expect(optionalLockup.burnRemainingOGV()).to.be.revertedWith(
          "Can not yet burn the remaining OGV"
        );
      }),
      it("should return true if proof is valid", async () => {
        let amount = bnDecimal(500000000);
        let valid = await optionalLockup
          .connect(claimer)
          .isProofValid(1, amount, claimer.address, merkleProof);
      ...
      it("should return false if proof is invalid", async () => {
        ...
        let valid = await optionalLockup
          .connect(claimer)
          .isProofValid(1, amount, claimer.address, invalidMerkleProof);
      ...
      it("should't be able to claim with invalid proof", async () => {
        ...
        await expect(
          optionalLockup
            .connect(claimer)
            .claim(1, amount, invalidMerkleProof, week)
