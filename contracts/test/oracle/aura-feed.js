...
describe("ForkTest: Aura/WETH Price Feed", function () {
  ...
  beforeEach(async () => {
    ...
    fixture.auraWETHWeightedPool = await ethers.getContract("MockOracleWeightedPool");

    await fixture.auraWETHPriceFeed.connect(
      fixture.governor
  ...
  it("should get Aura price from weighted pool", async () => {
    ...
    await auraWETHWeightedPool.connect(strategist).setNextResults([
      oethUnits("1"),
      oethUnits("1.01")
    ]);
    
    expect(await auraWETHPriceFeed.price()).to.eq(
      oethUnits("1.01")
    );
  ...
  it("Should allow governor to set tolerance value", async () => {
    const { auraWETHWeightedPool, auraWETHPriceFeed, governor } = fixture;

    const tx = await auraWETHPriceFeed.connect(governor).setTolerance(
      oethUnits("0.09"),
    )

    await expect(tx).to.emit(auraWETHPriceFeed, "ToleranceChanged")
      .withArgs(
        oethUnits("0.02").toString(),
        oethUnits("0.09").toString(),
      )

    expect(
      await auraWETHPriceFeed.tolerance()
    ).to.eq(oethUnits("0.09"))
  ...
  it("Should not allow strategist to set tolerance value", async () => {
    ...
    await expect(auraWETHPriceFeed.connect(strategist).setTolerance(
      oethUnits("0.09"),
    )).to.be.revertedWith("Caller is not the Governor")
  })
  ...
  it("Should not allow higher tolerance", async () => {
    ...
    await expect(auraWETHPriceFeed.connect(governor).setTolerance(
      oethUnits("0.11"),
  ...
  it("should be compatible with AggregatorV3Interface", async () => {
    ...
    await expect(auraWETHPriceFeed.getRoundData(1))
