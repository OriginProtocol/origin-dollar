...
describe("Liquidity Reward", function () {
  ...
  it("Deposit, withdraw, and claim with multiple accounts", async () => {
    ...
    expect(await liquidityRewardOUSD_USDT.totalOutstandingRewards()).to.approxEqual(
      rewardPerBlock.mul(8)
    );

    expect(
      await liquidityRewardOUSD_USDT.pendingRewards(anna._address)
    ).to.approxEqual(
      rewardPerBlock.mul(3) // first 3 blocks was anna alone
      .add( rewardPerBlock.mul(3).div(2) ) // next 3 was amongst 2
      .add( rewardPerBlock.mul(2).div(3) ) // next 2 was amongst 3
    );

    let mattRewards = rewardPerBlock.mul(3).div(2) // first 3 matt has to split with ann
    ...
    let annaReward =  rewardPerBlock.mul(3) // first 2 blocks was anna alone
    ...
    expect(await ogn.balanceOf(anna._address)).to.approxEqual(annaReward.add(baseOgn));
    ...
    expect(await ogn.balanceOf(josh._address)).to.approxEqual(
      rewardPerBlock.mul(4).div(3) // josh started late
