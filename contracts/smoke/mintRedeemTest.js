...
const assertExpectedOusd = (bigNumber, bigNumberExpected, tolerance=0.03) => {
  if (!isWithinTolerance(bigNumber, bigNumberExpected, 0.03)) {
    throw new Error(`Unexpected OUSD value. Expected ${ousdUnitsFormat(bigNumberExpected)} with the tolerance of ${tolerance}. Received: ${ousdUnitsFormat(bigNumber)}`);
  }
}

const assertExpectedStablecoins = (usdtBn, daiBn, usdcBn, unitsExpected, tolerance=0.03) => {
  ...
  if (!isWithinTolerance(allStablecoins, stableCoinsExpected, 0.03)) {
    throw new Error(`Unexpected value. Expected to receive total stablecoin units ${ousdUnitsFormat(stableCoinsExpected)} with the tolerance of ${tolerance}. Received: ${ousdUnitsFormat(allStablecoins)}`);
  }
}
...
const testMint = async (hre, beforeDeployData) => {
  ...
  if (!beforeDeployData.ousdAfterMint.eq(ousdBeforeMint)) {
    throw new Error(`Deploy changed the amount of ousd in user's account from ${ousdUnitsFormat(beforeDeployData.ousdAfterMint)} to ${ousdUnitsFormat(ousdBeforeMint)}`)
  }
...
const testRedeem = async (ousdAfterMint) => {
  ...
  assertExpectedOusd(ousdAfterRedeem, expectedOusd, 0.00);
...
const testTransfer = async () => {
  ...
  await ousd.connect(signer).transfer(signer2.address, ousdUnits(ousdToTransfer));

  const ousdSenderAfterSend = await getOusdBalance(signer);
  const ousdReceiverAfterSend = await getOusdBalance(signer2);

  assertExpectedOusd(ousdSenderAfterSend, ousdSenderBeforeSend.sub(ousdUnits(ousdToTransfer)), 0.00);
  assertExpectedOusd(ousdReceiverAfterSend, ousdReceiverBeforeSend.add(ousdUnits(ousdToTransfer)), 0.00);
}

const testMultipleMint = async () => {
  const amountToMint = '100'
  await usdt.connect(signer).approve(vault.address, usdtUnits(amountToMint), { gasLimit: 1000000 });
  await usdc.connect(signer).approve(vault.address, usdcUnits(amountToMint), { gasLimit: 1000000 });
  await dai.connect(signer).approve(vault.address, daiUnits(amountToMint), { gasLimit: 1000000 });

  const ousdBalanceBeforeMint = await getOusdBalance(signer);
  await vault.connect(signer).mintMultiple(
    [
      usdt.address,
      usdc.address,
      dai.address
  ...
  assertExpectedOusd(
    ...
    ousdBalanceBeforeMint.add(
      ousdUnits(amountToMint)
