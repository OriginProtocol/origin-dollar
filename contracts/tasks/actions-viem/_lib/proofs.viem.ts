// viem port of utils/proofs.js
//
// The proof engine is SSZ, not ethers. The only ethers usage in the original
// is `formatUnits` from "ethers/lib/utils", which is used purely for logging.
// Here it is replaced with viem's `formatUnits`. Every
// @chainsafe/persistent-merkle-tree call (createProof / concatGindices /
// toGindex / ProofType) and every gindex is kept byte-identical to the
// original, so the generated proofs are byte-identical.

import { formatUnits } from "viem";
import { toHex, concatProof } from "./beaconBytes";
import { MAX_UINT64 } from "../../../utils/constants";

const log = require("../../../utils/logger")("task:proof");

// The Lodestar SSZ views are complex generic types; the original JS treats them
// dynamically. We type them as `any` to keep the port faithful without coupling
// to specific Lodestar type signatures.
...
async function generateValidatorWithdrawableEpochProof({
  ...
  stateView,
  validatorIndex,
}: ProofInputs & { validatorIndex: number }) {
  // Have to dynamically import the Lodestar API client as its an ESM module
  const { concatGindices, createProof, ProofType, toGindex } = await import(
    "@chainsafe/persistent-merkle-tree"
  );
...
async function generatePendingDepositProof({
  ...
  stateView,
  depositIndex,
}: ProofInputs & { depositIndex: number }) {
  // Have to dynamically import the Lodestar API client as its an ESM module
  const { concatGindices, createProof, ProofType, toGindex } = await import(
    "@chainsafe/persistent-merkle-tree"
  );
  ...
  log(`  amount : ${formatUnits(BigInt(pendingDeposit.amount), 9)}`);
...
async function generateBalanceProof({
  ...
  log(
    `Validator ${validatorIndex} balance: ${formatUnits(
      BigInt(validatorBalance),
      9
    )}`
  );
