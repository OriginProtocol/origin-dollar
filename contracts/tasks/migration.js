...
async function marketplaceOwnership(taskArguments) {
  ...
  // Transfer ownership
  console.log(`Transferring ownership of ${marketplace1.address} from ${oldMultisigSigner.address} to ${newMultisigAddress}`)
  await marketplace0.connect(oldMultisigSigner).transferOwnership(newMultisigAddress)
  console.log(`Transferring ownership of ${marketplace1.address} from ${oldMultisigSigner.address} to ${newMultisigAddress}`)
  await marketplace1.connect(oldMultisigSigner).transferOwnership(newMultisigAddress)
