...
const { KMSClient, EncryptCommand, DecryptCommand } = require("@aws-sdk/client-kms"); // CommonJS import
...
const encryptMasterPrivateKey = async ({ privateKey }) => {
  ...
  const hexEncodedPrivateKey = Buffer.from(response.CiphertextBlob).toString('hex');
  console.log("Hex encoded private key: ", hexEncodedPrivateKey)
};

const decryptMasterPrivateKey = async () => {
  const hexEncodedMasterPrivateKey = process.env.VALIDATOR_MASTER_ENCRYPTED_PRIVATE_KEY;

  const client = new KMSClient();
  const uintArrayHex = Uint8Array.from(Buffer.from(hexEncodedMasterPrivateKey, 'hex'));
