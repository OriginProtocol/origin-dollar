 ...
 *
 *         We resolve the destination strategy from the (sourceDomain, sender) pair, unwrap
 *         the envelope, transfer USDC to that strategy, and call `receiveFromBridge`.
 */
contract CCTPInboundAdapter is AbstractInboundAdapter, IMessageHandlerV2 {
    ...
    constructor(address _usdcToken, address _cctpMessageTransmitter) {
        require(_usdcToken != address(0), "CCTPIn: zero usdc");
        require(_cctpMessageTransmitter != address(0), "CCTPIn: zero mt");
    ...
    modifier onlyCCTP() {
        require(
            ...
            "CCTPIn: not message transmitter"
        );
        _;
    }
    ...
    function handleReceiveUnfinalizedMessage(
        ...
        revert("CCTPIn: unfinalised not accepted");
    }
    ...
    function _validateAndDeliver(
        ...
        address peer = address(uint160(uint256(sender)));
        address strategy = strategyFor[uint64(sourceDomain)][peer];
        require(strategy != address(0), "CCTPIn: unknown peer");
