...
contract ConvexGeneralizedMetaStrategy is BaseConvexMetaStrategy {
    ...
    function _lpDepositAll() internal override {
         ...
         */
        uint256 minReceived =
            threePoolLpDollarValue.divPrecisely(metapoolVirtualPrice).mulTruncate(uint256(1e18) - MAX_SLIPPAGE);
    ...
    function _lpWithdraw(uint256 num3CrvTokens) internal override {
        ...
        uint256 requiredMetapoolLpTokens = _calcCurveMetaTokenAmount(crvCoinIndex, num3CrvTokens);
        ...
        IRewardStaking(cvxRewardStakerAddress).withdrawAndUnwrap(requiredMetapoolLpTokens, true);

        if (requiredMetapoolLpTokens > 0) {
            // slither-disable-next-line unused-return
            metapool.remove_liquidity_one_coin(requiredMetapoolLpTokens, int128(crvCoinIndex), num3CrvTokens);
        }
    }
    ...
    function _lpWithdrawAll() internal override {
        ...
        if (gaugeTokens > 0) {
            uint256 burnDollarAmount = gaugeTokens.mulTruncate(metapool.get_virtual_price());
            ...
            metapool.remove_liquidity_one_coin(
                gaugeTokens,
                int128(crvCoinIndex),
                curve3PoolExpected - curve3PoolExpected.mulTruncate(maxWithdrawalSlippage)
            );
        }
    }
}
