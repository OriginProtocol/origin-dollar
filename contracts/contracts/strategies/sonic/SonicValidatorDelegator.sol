...
abstract contract SonicValidatorDelegator is InitializableAbstractStrategy {
    ...
    modifier onlyRegistratorOrStrategist() {
        require(
            msg.sender == validatorRegistrator || msg.sender == IVault(vaultAddress).strategistAddr(),
    ...
    function checkBalance(address _asset) external view virtual override returns (uint256 balance) {
        ...
        balance = IERC20(wrappedSonic).balanceOf(address(this)) + pendingWithdrawals;
    ...
    function _delegate(uint256 _amount) internal {
        require(isSupportedValidator(defaultValidatorId), "Validator not supported");
    ...
    function _undelegate(uint256 _validatorId, uint256 _undelegateAmount) internal returns (uint256 withdrawId) {
        ...
        require(_undelegateAmount <= amountDelegated, "Insufficient delegation");

        withdrawId = nextWithdrawId++;

        withdrawals[withdrawId] = WithdrawRequest(_validatorId, _undelegateAmount, block.timestamp);
        ...
        returns (uint256 withdrawnAmount)
    {
        ...
        // continue below
            ...
            if (errorSelector == ISFC.StakeIsFullySlashed.selector) {
                ...
                emit Withdrawn(_withdrawId, withdrawal.validatorId, withdrawal.undelegatedAmount, withdrawnAmount);
    ...
    /// @param _withdrawId The unique withdraw ID used to `undelegate`
    function isWithdrawnFromSFC(uint256 _withdrawId) public view returns (bool) {
        WithdrawRequest memory withdrawal = withdrawals[_withdrawId];
     ...
     */
    function restakeRewards(uint256[] calldata _validatorIds) external nonReentrant {
        for (uint256 i = 0; i < _validatorIds.length; ++i) {
            require(isSupportedValidator(_validatorIds[i]), "Validator not supported");

            uint256 rewards = sfc.pendingRewards(address(this), _validatorIds[i]);
     ...
     */
    function collectRewards(uint256[] calldata _validatorIds) external onlyRegistratorOrStrategist nonReentrant {
        uint256 sBalanceBefore = address(this).balance;
    ...
    receive() external payable {
        require(msg.sender == address(sfc) || msg.sender == wrappedSonic, "S not from allowed contracts");
    }
    ...
    ****************************************/
    ...
    function setRegistrator(address _validatorRegistrator) external onlyGovernor {
    ...
    function setDefaultValidatorId(uint256 _validatorId) external onlyRegistratorOrStrategist {
    ...
    function supportValidator(uint256 _validatorId) external onlyGovernor {
        require(!isSupportedValidator(_validatorId), "Validator already supported");
    ...
    function isSupportedValidator(uint256 _validatorId) public view returns (bool) {
        uint256 validatorLen = supportedValidators.length;
        for (uint256 i = 0; i < validatorLen; ++i) {
            if (supportedValidators[i] == _validatorId) {
                return true;
