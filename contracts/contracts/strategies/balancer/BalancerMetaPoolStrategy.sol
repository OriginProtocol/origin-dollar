...
contract BalancerMetaPoolStrategy is BaseAuraStrategy {
     ...
     * functionality.
     */
    function deposit(address[] calldata, uint256[] calldata) external onlyVault nonReentrant {
        revert("Not supported");
    }
     ...
     */
    function _deposit(address[] memory _strategyAssets, uint256[] memory _strategyAmounts) internal {
        require(_strategyAssets.length == _strategyAmounts.length, "Array length missmatch");

        (IERC20[] memory tokens,,) = balancerVault.getPoolTokens(balancerPoolId);

        uint256[] memory strategyAssetAmountsToPoolAssetAmounts = new uint256[](_strategyAssets.length);
        ...
        for (uint256 i = 0; i < _strategyAssets.length; ++i) {
            ...
            require(assetToPToken[strategyAsset] != address(0), "Unsupported asset");
            ...
            if (strategyAmount > 0) {
                ...
                (, strategyAssetAmountsToPoolAssetAmounts[i]) = _wrapPoolAsset(strategyAsset, strategyAmount);
            }
        }
        ...
        uint256 minBPT = _getBPTExpected(strategyAssetsToPoolAssets, strategyAssetAmountsToPoolAssetAmounts);
        ...
        bytes memory userData =
            abi.encode(IBalancerVault.WeightedPoolJoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT, amountsIn, minBPTwDeviation);

        IBalancerVault.JoinPoolRequest memory request =
            IBalancerVault.JoinPoolRequest(poolAssets, amountsIn, userData, false);
     ...
     */
    function withdraw(address _recipient, address _strategyAsset, uint256 _strategyAmount)
        external
        override
        onlyVault
        nonReentrant
     ...
     */
    function withdraw(address _recipient, address[] calldata _strategyAssets, uint256[] calldata _strategyAmounts)
        external
        onlyVault
        nonReentrant
    {
     ...
     * already be in the Balancer Vault context and wouldn't be able to enter it again.
     */
    function _withdraw(address _recipient, address[] memory _strategyAssets, uint256[] memory _strategyAmounts)
        internal
    {
        require(_strategyAssets.length == _strategyAmounts.length, "Invalid input arrays");
        ...
        uint256[] memory strategyAssetsToPoolAssetsAmounts = new uint256[](_strategyAssets.length);
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            ...
            for (uint256 j = 0; j < _strategyAssets.length; ++j) {
                ...
                if (_strategyAssets[j] == strategyAsset) {
                    (, poolAssetsAmountsOut[i]) = _toPoolAsset(strategyAsset, _strategyAmounts[j]);
                    strategyAssetsToPoolAssetsAmounts[j] = poolAssetsAmountsOut[i];
        ...
        maxBPTtoWithdraw = maxBPTtoWithdraw.mulTruncate(1e18 + maxWithdrawalDeviation);
        ...
        _lpWithdraw(maxBPTtoWithdraw - IERC20(platformAddress).balanceOf(address(this)));
        ...
        bytes memory userData = abi.encode(
            IBalancerVault.WeightedPoolExitKind.BPT_IN_FOR_EXACT_TOKENS_OUT, poolAssetsAmountsOut, maxBPTtoWithdraw
        );
        ...
        for (uint256 i = 0; i < _strategyAssets.length; ++i) {
            ...
            if (strategyAssetsToPoolAssetsAmounts[i] > 0) {
                _unwrapPoolAsset(_strategyAssets[i], strategyAssetsToPoolAssetsAmounts[i]);
            }

            // Transfer the vault collateral assets to the recipient, which is typically the vault
            if (_strategyAmounts[i] > 0) {
                IERC20(_strategyAssets[i]).safeTransfer(_recipient, _strategyAmounts[i]);

                emit Withdrawal(_strategyAssets[i], platformAddress, _strategyAmounts[i]);
            }
        }
    }
    ...
    function withdrawAll() external override onlyVaultOrGovernor nonReentrant {
        ...
        uint256 BPTtoWithdraw = IERC20(platformAddress).balanceOf(address(this));
         ...
         */
        bytes memory userData =
            abi.encode(IBalancerVault.WeightedPoolExitKind.EXACT_BPT_IN_FOR_TOKENS_OUT, BPTtoWithdraw);

        IBalancerVault.ExitPoolRequest memory request =
            IBalancerVault.ExitPoolRequest(poolAssets, minAmountsOut, userData, false);
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            ...
            uint256 poolAssetAmount = IERC20(poolAsset).balanceOf(address(this));

            // Unwrap assets like wstETH and sfrxETH to rebasing assets stETH and frxETH
            uint256 unwrappedAmount = 0;
            if (poolAssetAmount > 0) {
                unwrappedAmount = _unwrapPoolAsset(strategyAsset, poolAssetAmount);
            }

            // Transfer the vault collateral assets to the vault
            if (unwrappedAmount > 0) {
                IERC20(strategyAsset).safeTransfer(vaultAddress, unwrappedAmount);
                emit Withdrawal(strategyAsset, platformAddress, unwrappedAmount);
            }
        }
    }
     ...
     */
    function _getRateProviderRate(address _asset) internal view override returns (uint256) {
