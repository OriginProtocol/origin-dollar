...
contract GeneralizedUniswapV3Strategy is InitializableAbstractStrategy {
    ...
    event ReserveStrategyChanged(address indexed token, address strategy);
    event UniswapV3FeeCollected(uint256 indexed tokenId, uint256 amount0, uint256 amount1);
    event UniswapV3LiquidityAdded(uint256 indexed tokenId, uint256 amount0Sent, uint256 amount1Sent, uint128 liquidityMinted);
    event UniswapV3LiquidityRemoved(uint256 indexed tokenId, uint256 amount0Received, uint256 amount1Received, uint128 liquidityBurned);
    event UniswapV3PositionClosed(uint256 indexed tokenId, uint256 amount0Received, uint256 amount1Received, uint128 liquidityBurned);
    ...
    uint24 internal constant MAX_SLIPPAGE = 100; // 1%
    ...
    */
    modifier onlyGovernorOrStrategistOrOperator() {
        require(
            msg.sender == operatorAddr || msg.sender == IVault(vaultAddress).strategistAddr() || isGovernor(),
    ...
    function setReserveStrategy(address _token0ReserveStrategy, address _token1ReserveStrategy) external onlyGovernorOrStrategistOrOperator nonReentrant {
        _setReserveStrategy(_token0ReserveStrategy, _token1ReserveStrategy);
    }

    function _setReserveStrategy(address _token0ReserveStrategy, address _token1ReserveStrategy) internal {
        // require(IStrategy(_token0ReserveStrategy).supportsAsset(token0), "Invalid Reserve Strategy");
        // require(IStrategy(_token1ReserveStrategy).supportsAsset(token1), "Invalid Reserve Strategy");

        reserveStrategy[token0] = _token0ReserveStrategy;
        reserveStrategy[token1] = _token1ReserveStrategy;

        emit ReserveStrategyChanged(token0, _token0ReserveStrategy);
        emit ReserveStrategyChanged(token1, _token1ReserveStrategy);
    }
    ...
    function withdraw(
        ...
        uint256 reserveBalance = IStrategy(reserveStrategy[asset]).checkBalance(asset);
        if (reserveBalance >= amount) {
            IVault(vaultAddress).withdrawForUniswapV3(recipient, asset, amount);
            return;
        }

        uint256 amountToPullOut = amount - reserveBalance;
    ...
    function getPendingRewards() external view returns (uint128 amount0, uint128 amount1) {
        if (currentPositionTokenId > 0) {
            (amount0, amount1) = _getTokensOwed(currentPositionTokenId);
        }
    }

    function _getTokensOwed(uint256 tokenId) internal view returns (uint128 tokensOwed0, uint128 tokensOwed1) {
        ...
        returns (uint256 balance)
    {
        ...
        (uint160 sqrtRatioX96, , , , , ,) = IUniswapV3Pool(platformAddress).slot0();
    ...
    function _checkAssetBalanceOfPosition(address asset, Position memory p, uint160 sqrtRatioX96) internal view returns (uint256 balance) {
        if (asset == token0) {
    ...
    function _checkBalanceOfPosition(Position memory p, uint160 sqrtRatioX96) internal view returns (uint256 amount0, uint256 amount1) {
        if (p.liquidity == 0) {
        ...
        (uint128 feeAmount0, uint128 feeAmount1) = _getTokensOwed(p.tokenId);

        amount0 += feeAmount0;
        amount1 += feeAmount1;
    }

    function rebalance(uint256 maxAmount0, uint256 maxAmount1, int24 lowerTick, int24 upperTick) external onlyGovernorOrStrategistOrOperator nonReentrant {
        if (currentPositionTokenId > 0) {
            _closePosition(currentPositionTokenId);
        }
        ...
        if (token0Balance < maxAmount0) {
            vault.withdrawForUniswapV3(address(this), token0, maxAmount0 - token0Balance);
        }

        uint256 token1Balance = cToken1.balanceOf(address(this));
        if (token1Balance < maxAmount1) {
            vault.withdrawForUniswapV3(address(this), token1, maxAmount1 - token1Balance);
        }
        ...
        uint256 token0Dust = cToken0.balanceOf(address(this));
        if (token0Dust > 0) {
            vault.depositForUniswapV3(token0, token0Dust);
        }

        uint256 token1Dust = cToken1.balanceOf(address(this));
        if (token1Dust > 0) {
            vault.depositForUniswapV3(token1, token1Dust);
        }

        currentPositionTokenId = tokenId;
    }

    function closeActivePosition() external onlyGovernorOrStrategistOrOperator nonReentrant {
        require(currentPositionTokenId > 0, "No active position");
        _closePosition(currentPositionTokenId);
    }

    function closePosition(uint256 tokenId) external onlyGovernorOrStrategistOrOperator nonReentrant {
        require(tokenIdToPosition[tokenId].liquidity > 0, "Invalid position");
        _closePosition(tokenId);
    }

    function _getTickPositionKey(int24 lowerTick, int24 upperTick) internal returns (int48 key) {
    ...
    function _closePosition(uint256 tokenId) internal returns (uint256 amount0, uint256 amount1) {
        ...
        (uint256 amount0Fee, uint256 amount1Fee) = _collectFeesForToken(tokenId);
    ...
    function _collectFeesForToken(uint256 tokenId) internal returns (uint256 amount0, uint256 amount1) {
    ...
    function _mintPosition(uint256 maxAmount0, uint256 maxAmount1, int24 lowerTick, int24 upperTick) internal returns (uint256 tokenId, uint128 liquidity, uint256 amount0, uint256 amount1) {
        INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                ...
                amount0Min: maxAmount0 * (10000 - MAX_SLIPPAGE), // 1% Slippage,
                amount1Min: maxAmount1 * (10000 - MAX_SLIPPAGE), // 1% Slippage,
                recipient: address(this),
    ...
    function _increaseLiquidityForPosition(Position storage p, uint256 maxAmount0, uint256 maxAmount1) internal returns (uint128 liquidity, uint256 amount0, uint256 amount1) {
    ...
    function _decreaseLiquidityForPosition(Position storage p, uint128 liquidity) internal returns (uint256 amount0, uint256 amount1) {
        (uint160 sqrtRatioX96, , , , , ,) = IUniswapV3Pool(platformAddress).slot0();
        (uint256 exactAmount0, uint256 exactAmount1) = uniswapV3Helper.getAmountsForLiquidity(
            sqrtRatioX96,
            p.sqrtRatioAX96, 
            p.sqrtRatioBX96, 
            liquidity
        ...
        INonfungiblePositionManager.DecreaseLiquidityParams memory params =
            INonfungiblePositionManager.DecreaseLiquidityParams({
                tokenId: p.tokenId,
                liquidity: liquidity,
                amount0Min: exactAmount0 * (10000 - MAX_SLIPPAGE), // 1% Slippage
                amount1Min: exactAmount1 * (10000 - MAX_SLIPPAGE), // 1% Slippage
                deadline: block.timestamp
            });
    ...
    function _abstractSetPToken(address _asset, address _pToken)
        ...
        override
    {
        IERC20(_asset).safeApprove(address(nonfungiblePositionManager), type(uint256).max);
    }
    ...
    function setPTokenAddress(address _asset, address _pToken) external override onlyGovernor {
        /**
