...
contract Timelock {
        ...
        returns (bytes32)
    {
        ...
        bytes32 txHash = keccak256(abi.encode(target, signature, keccak256(data), eta));
    ...
    function cancelTransaction(address target, string memory signature, bytes memory data, uint256 eta) internal {
    ...
    function pauseCapital(address target) external {
        require(msg.sender == admin, "Timelock::pauseCapital: Call must come from admin.");
        CapitalPausable(target).pauseCapital();
    }

    function unpauseCapital(address target) external {
        require(msg.sender == admin, "Timelock::unpauseCapital: Call must come from admin.");
