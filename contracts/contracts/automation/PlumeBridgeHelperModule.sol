...
contract PlumeBridgeHelperModule is AccessControlEnumerable, AbstractLZBridgeHelperModule {
    IVault public constant vault = IVault(0xc8c8F8bEA5631A8AF26440AF32a55002138cB76a);
    IWETH9 public constant weth = IWETH9(0xca59cA09E5602fAe8B629DeE83FfA819741f14be);
    IERC20 public constant oethp = IERC20(0xFCbe50DbE43bF7E5C88C6F6Fb9ef432D4165406E);
    ...
    IOFT public constant LZ_WOETH_OMNICHAIN_ADAPTER = IOFT(0x592CB6A596E7919930bF49a27AdAeCA7C055e4DB);
    IOFT public constant LZ_ETH_OMNICHAIN_ADAPTER = IOFT(0x4683CE822272CD66CEa73F5F1f9f5cBcaEF4F066);
     ...
     */
    function bridgeWOETHToEthereum(uint256 woethAmount, uint256 slippageBps) public payable onlyOperator {
        _bridgeTokenWithLz(
     ...
     * @param slippageBps Slippage in 10^4 basis points.
     */
    function bridgeWETHToEthereum(uint256 wethAmount, uint256 slippageBps) public payable onlyOperator {
        _bridgeTokenWithLz(
            LZ_ETHEREUM_ENDPOINT_ID, IERC20(address(weth)), LZ_ETH_OMNICHAIN_ADAPTER, wethAmount, slippageBps, false
        );
    }
     ...
     */
    function depositWOETH(uint256 woethAmount, bool redeemWithVault) external onlyOperator returns (uint256) {
     ...
     * @return Amount of OETHp received.
     */
    function _depositWOETH(uint256 woethAmount, bool redeemWithVault) internal returns (uint256) {
        // Update oracle price
        ...
        success = safeContract.execTransactionFromModule(
            address(vault),
            0, // Value
            abi.encodeWithSelector(bytes4(keccak256("redeem(uint256,uint256)")), oethpAmount, oethpAmount),
            0 // Call
        );
    ...
    function _withdrawWOETH(uint256 wethAmount) internal returns (uint256) {
        ...
        success = safeContract.execTransactionFromModule(
            ...
            abi.encodeWithSelector(
                bytes4(keccak256("mint(address,uint256,uint256)")), address(weth), wethAmount, wethAmount
            ),
            0 // Call
        );
        ...
        success = safeContract.execTransactionFromModule(
            address(oethp),
            0, // Value
            abi.encodeWithSelector(oethp.approve.selector, address(bridgedWOETHStrategy), wethAmount),
            0 // Call
        );
