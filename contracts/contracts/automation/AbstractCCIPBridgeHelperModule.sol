...
abstract contract AbstractCCIPBridgeHelperModule is AbstractSafeModule {
    ...
    function _bridgeTokenWithCCIP(
        ...
        Client.EVMTokenAmount memory tokenAmount = Client.EVMTokenAmount({token: address(token), amount: amount});
        ...
        // Get CCIP fee
        uint256 ccipFee = ccipRouter.getFee(destinationChainSelector, ccipMessage);

        // Send CCIP message
        success = safeContract.execTransactionFromModule(
            address(ccipRouter),
            ccipFee, // Value
            abi.encodeWithSelector(ccipRouter.ccipSend.selector, destinationChainSelector, ccipMessage),
            0 // Call
        );
