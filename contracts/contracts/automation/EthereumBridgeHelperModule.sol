...
contract EthereumBridgeHelperModule is AccessControlEnumerable, AbstractCCIPBridgeHelperModule {
    IVault public constant vault = IVault(0x39254033945AA2E4809Cc2977E7087BEE48bd7Ab);
    ...
    IRouterClient public constant CCIP_ROUTER = IRouterClient(0x80226fc0Ee2b096224EeAc085Bb9a8cba1146f7D);
     ...
     */
    function bridgeWOETHToBase(uint256 woethAmount) public payable onlyOperator {
        _bridgeTokenWithCCIP(CCIP_ROUTER, CCIP_BASE_CHAIN_SELECTOR, woeth, woethAmount);
    }
    ...
    function bridgeWETHToBase(uint256 wethAmount) public payable onlyOperator {
        _bridgeTokenWithCCIP(CCIP_ROUTER, CCIP_BASE_CHAIN_SELECTOR, IERC20(address(weth)), wethAmount);
    }
     ...
     */
    function mintAndWrap(uint256 wethAmount, bool useNativeToken) external onlyOperator returns (uint256) {
        if (useNativeToken) {
            wrapETH(wethAmount);
        }
    ...
    function _mintAndWrap(uint256 wethAmount) internal returns (uint256) {
        ...
        success = safeContract.execTransactionFromModule(
            address(oeth),
            0, // Value
            abi.encodeWithSelector(oeth.approve.selector, address(woeth), wethAmount),
            0 // Call
        );
        ...
        success = safeContract.execTransactionFromModule(
            address(woeth),
            0, // Value
            abi.encodeWithSelector(woeth.deposit.selector, wethAmount, address(safeContract)),
            0 // Call
        );
     ...
     * @param useNativeToken Whether to use native token to mint.
     */
    function mintWrapAndBridgeToBase(uint256 wethAmount, bool useNativeToken) external payable onlyOperator {
        if (useNativeToken) {
            wrapETH(wethAmount);
        }
     ...
     */
    function claimAndBridgeToBase(uint256 requestId) external payable onlyOperator {
     ...
     * @return oethAmount Amount of OETH queued for withdrawal.
     */
    function _unwrapAndRequestWithdrawal(uint256 woethAmount) internal returns (uint256 requestId, uint256 oethAmount) {
        ...
        bool success = safeContract.execTransactionFromModule(
            address(woeth),
            0, // Value
            abi.encodeWithSelector(woeth.redeem.selector, woethAmount, address(safeContract), address(safeContract)),
            0 // Call
        );
        ...
        success = safeContract.execTransactionFromModule(
            address(vault),
            0, // Value
            abi.encodeWithSelector(vault.requestWithdrawal.selector, oethAmount),
            0 // Call
        );
