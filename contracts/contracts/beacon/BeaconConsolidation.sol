...
library BeaconConsolidation {
    ...
    function request(bytes calldata source, bytes calldata target) internal returns (uint256 fee_) {
        ...
        (bool success,) = CONSOLIDATION_REQUEST_ADDRESS.call{value: fee_}(abi.encodePacked(source, target));
    ...
    function fee() internal view returns (uint256) {
        ...
        (bool success, bytes memory result) = CONSOLIDATION_REQUEST_ADDRESS.staticcall("");
