...
contract MockBeaconRoots {
    ...
    function parentBlockRoot(uint64 timestamp) external view returns (bytes32 parentRoot) {
        return BeaconRoots.parentBlockRoot(timestamp);
    }

    function latestBlockRoot() external view returns (bytes32 parentRoot, uint64 timestamp) {
