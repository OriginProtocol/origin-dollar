...
contract MockCurvePool {
    ...
    // solhint-disable-next-line no-unused-vars
    function remove_liquidity(uint256 _lpAmount, uint256[3] memory _min_amounts) public {
        // Burn the Curve LP tokens
        IBurnableERC20(lpToken).burnFrom(msg.sender, _lpAmount);
        uint256 totalSupply = IERC20(lpToken).totalSupply();
        for (uint256 i = 0; i < 3; i++) {
            uint256 coinAmount = totalSupply > 0
                ? (_lpAmount * IERC20(coins[i]).balanceOf(address(this))) / totalSupply
    ...
    function remove_liquidity_imbalance(uint256[3] memory _amounts, uint256 _max_burned_tokens) public {
        // Burn the Curve LP tokens
    ...
    function calc_token_amount(uint256[3] memory _amounts, bool) public view returns (uint256 lpTokens) {
        for (uint256 i = 0; i < _amounts.length; i++) {
            uint256 assetDecimals = Helpers.getDecimals(coins[i]);
    ...
    function exchange(uint256 coin0, uint256 coin1, uint256 amountIn, uint256 minAmountOut)
        external
