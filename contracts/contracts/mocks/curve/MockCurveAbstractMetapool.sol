...
abstract contract MockCurveAbstractMetapool is MintableERC20 {
    ...
    function add_liquidity(uint256[2] calldata _amounts, uint256 _minAmount) external returns (uint256 lpAmount) {
        for (uint256 i = 0; i < _amounts.length; i++) {
            if (_amounts[i] > 0) {
                IERC20(coins[i]).transferFrom(msg.sender, address(this), _amounts[i]);
        ...
        returns (uint256[2] memory amounts)
    {
        ...
        for (uint256 i = 0; i < 2; i++) {
            amounts[i] = totalSupply > 0
                ? (_amount * IERC20(coins[i]).balanceOf(address(this))) / totalSupply
    ...
    function remove_liquidity_imbalance(uint256[2] memory _amounts, uint256 _max_burned_tokens)
        public
        returns (uint256)
    {
        return _remove_liquidity_imbalance(_amounts, _max_burned_tokens, msg.sender);
    }

    function remove_liquidity_imbalance(uint256[2] memory _amounts, uint256 _max_burned_tokens, address _reveiver)
        public
        returns (uint256)
    {
        return _remove_liquidity_imbalance(_amounts, _max_burned_tokens, _reveiver);
    }

    function _remove_liquidity_imbalance(uint256[2] memory _amounts, uint256 _max_burned_tokens, address _reveiver)
        internal
    ...
    function calc_token_amount(uint256[2] memory _amounts, bool) public view returns (uint256 lpTokens) {
        for (uint256 i = 0; i < _amounts.length; i++) {
            uint256 assetDecimals = Helpers.getDecimals(coins[i]);
