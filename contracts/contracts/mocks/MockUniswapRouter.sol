...
contract MockUniswapRouter is IUniswapV2Router {
    ...
    function initialize(address[] calldata _0tokens, address[] calldata _1tokens) public {
        require(_0tokens.length == _1tokens.length, "Mock token pairs should be of the same length");
    ...
    function exactInput(ExactInputParams calldata params) external payable returns (uint256 amountOut) {
        ...
        require(amountOut >= params.amountOutMinimum, "UniswapMock: amountOut less than amountOutMinimum");
        return amountOut;
    }
    ...
    function execute(bytes calldata, bytes[] calldata inputs, uint256) external payable {
        ...
        for (uint256 i = 0; i < inLen; ++i) {
            (address recipient,, uint256 amountOutMinimum, bytes memory path,) =
                abi.decode(inputs[i], (address, uint256, uint256, bytes, bool));

            (address token0, address token1) = _getFirstAndLastToken(path);

            amountOutMinimum = amountOutMinimum.scaleBy(Helpers.getDecimals(token0), Helpers.getDecimals(token1));

            MintableERC20(token1).mintTo(recipient, amountOutMinimum);
        }
    }

    function _getFirstAndLastToken(bytes memory path) internal view returns (address token0, address token1) {
