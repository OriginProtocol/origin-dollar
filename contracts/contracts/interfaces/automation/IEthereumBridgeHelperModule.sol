...
interface IEthereumBridgeHelperModule is IAbstractSafeModule {
    ...
    function mintAndWrap(uint256 wethAmount, bool useNativeToken) external returns (uint256);

    function wrapETH(uint256 ethAmount) external payable;

    function mintWrapAndBridgeToBase(uint256 wethAmount, bool useNativeToken) external payable;

    function unwrapAndRequestWithdrawal(uint256 woethAmount) external returns (uint256 requestId, uint256 oethAmount);
