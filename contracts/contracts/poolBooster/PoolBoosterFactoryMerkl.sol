...
contract PoolBoosterFactoryMerkl is AbstractPoolBoosterFactory {
    ...
    constructor(address _oToken, address _governor, address _centralRegistry, address _beacon)
    ...
    function createPoolBoosterMerkl(address _ammPoolAddress, bytes calldata _initData, uint256 _salt)
        external
        onlyGovernor
    {
        require(_ammPoolAddress != address(0), "Invalid ammPoolAddress address");
        require(_salt > 0, "Invalid salt");
        require(poolBoosterFromPool[_ammPoolAddress].boosterAddress == address(0), "Pool booster already exists");

        address proxy = address(new BeaconProxy{salt: bytes32(_salt)}(beacon, _initData));

        _storePoolBoosterEntry(proxy, _ammPoolAddress, IPoolBoostCentralRegistry.PoolBoosterType.MerklBooster);
    }
    ...
    function bribeAll(address[] memory _exclusionList) public override onlyGovernor {
    ...
    function removePoolBooster(address _poolBoosterAddress) external override onlyGovernor {
    ...
    function computePoolBoosterAddress(uint256 _salt, bytes calldata _initData) external view returns (address) {
        require(_salt > 0, "Invalid salt");

        bytes memory bytecode = abi.encodePacked(type(BeaconProxy).creationCode, abi.encode(beacon, _initData));
        bytes32 hash = keccak256(abi.encodePacked(bytes1(0xff), address(this), bytes32(_salt), keccak256(bytecode)));
