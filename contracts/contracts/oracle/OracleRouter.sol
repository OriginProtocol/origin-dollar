...
abstract contract OracleRouterBase is IOracle {
     ...
     */
    function feedMetadata(address asset) internal view virtual returns (
        address feedAddress,
...
contract OracleRouter is OracleRouterBase {
    ...
    {
        /* + 5 minutes is added just in case the transaction would want to execute exactly
         * at the moment of the heartbeat. And the heartbeat transaction hasn't yet been
         * mined. 
         */
        if (asset == 0x6B175474E89094C44Da98b954EedeAC495271d0F) {
            ...
            maxDataStaleness = 1 hours + 5 minutes;
            ...
            maxDataStaleness = 1 days + 5 minutes;
...
contract OETHOracleRouter is OracleRouter {
        ...
        returns (uint256)
    {
        ...
        console.log("maxStaleness", maxStaleness);
...
contract OracleRouterDev is OracleRouterBase {
    ...
    function setFeed(address _asset, address _feed, uint256 _updatedAt) external {
