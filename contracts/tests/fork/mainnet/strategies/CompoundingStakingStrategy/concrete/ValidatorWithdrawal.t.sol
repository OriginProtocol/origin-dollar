    ...
    Fork_CompoundingStakingStrategy_Shared_Test
{
    ...
    uint64 internal constant PARTIAL_WITHDRAWAL_AMOUNT_GWEI = uint64(1 ether / 1 gwei);

    function test_validatorWithdrawal_partialCallsLiveWithdrawalRequestContract() public {
        bytes32 pubKeyHash = sha256(abi.encodePacked(ACTIVE_VALIDATOR_PUBKEY, bytes16(0)));
        _assertValidatorState(pubKeyHash, CompoundingValidatorState.ACTIVE);

        uint256 fee = _withdrawalRequestFee();
        uint256 requestContractBalanceBefore = Mainnet.beaconChainWithdrawRequest.balance;
        ...
        strategy.validatorWithdrawal{value: fee}(ACTIVE_VALIDATOR_PUBKEY, PARTIAL_WITHDRAWAL_AMOUNT_GWEI);
    ...
    function test_validatorWithdrawal_fullCallsLiveWithdrawalRequestContract() public {
        bytes32 pubKeyHash = sha256(abi.encodePacked(ACTIVE_VALIDATOR_PUBKEY, bytes16(0)));
        _assertValidatorState(pubKeyHash, CompoundingValidatorState.ACTIVE);
    ...
    function _assertValidatorState(bytes32 pubKeyHash, CompoundingValidatorState expectedState) internal view {
        (CompoundingValidatorState state,) = strategy.validator(pubKeyHash);
        assertEq(uint256(state), uint256(expectedState), "unexpected validator state");
    }

    function _withdrawalRequestFee() internal view returns (uint256 fee) {
        (bool success, bytes memory result) = Mainnet.beaconChainWithdrawRequest.staticcall("");
        require(success && result.length > 0, "failed to get withdrawal request fee");
