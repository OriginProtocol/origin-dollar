...
contract Fork_Concrete_MorphoV2Strategy_Deposit_Test is
    Fork_MorphoV2Strategy_Shared_Test
{
    function test_deposit_sendsAssetsToRealMorphoVault() public {
        uint256 amount = 1000e6;

        uint256 sharesBefore = IERC20(Mainnet.MorphoOUSDv2Vault).balanceOf(
            address(strategy)
        );
        _depositAsVault(amount);
        uint256 sharesAfter = IERC20(Mainnet.MorphoOUSDv2Vault).balanceOf(
            address(strategy)
        );
    ...
    function test_deposit_emitsDepositEvent() public {
        ...
        emit IMorphoV2Strategy.Deposit(
            Mainnet.USDC,
            Mainnet.MorphoOUSDv2Vault,
            amount
        );
