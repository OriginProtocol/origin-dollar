...
contract Fork_Concrete_SonicStakingStrategy_InitialState_Test is Fork_SonicStakingStrategy_Shared_Test {
    function test_initialState() public view {
        assertEq(sonicStakingStrategy.wrappedSonic(), address(wrappedSonic), "Incorrect wrapped sonic address");
        ...
        for (uint256 i = 0; i < testValidatorIds.length; i++) {
            assertTrue(
                sonicStakingStrategy.isSupportedValidator(testValidatorIds[i]), "Validator expected to be supported"
            );
        }
