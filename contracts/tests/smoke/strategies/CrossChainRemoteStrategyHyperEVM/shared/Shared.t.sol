...
abstract contract Smoke_CrossChainRemoteStrategyHyperEVM_Shared_Test is
    BaseSmoke
{
    //////////////////////////////////////////////////////
    /// --- ADDRESSES
    ...
    function setUp() public virtual override {
        ...
        require(
            address(resolver).code.length > 0,
            "Resolver not initialized on fork"
        );
        crossChainRemoteStrategy = CrossChainRemoteStrategy(
            resolver.resolve("CROSS_CHAIN_REMOTE_STRATEGY")
        );
    ...
    function _replaceMessageTransmitter()
        internal
        returns (CCTPMessageTransmitterMock2)
    {
        CCTPMessageTransmitterMock2 temp = new CCTPMessageTransmitterMock2(
            HyperEVM.USDC,
            0
        );
        vm.etch(CrossChain.CCTPMessageTransmitterV2, address(temp).code);

        CCTPMessageTransmitterMock2 mock = CCTPMessageTransmitterMock2(
