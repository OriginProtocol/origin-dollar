...
contract Smoke_CrossChainRemoteStrategyHyperEVM_Withdraw_Test is Smoke_CrossChainRemoteStrategyHyperEVM_Shared_Test {
    function test_withdraw_handlesIncomingWithdraw() public {
        ...
        crossChainRemoteStrategy.deposit(HyperEVM.USDC, depositAmount);

        // Snapshot state
        uint256 balanceBefore = crossChainRemoteStrategy.checkBalance(HyperEVM.USDC);
        ...
        // Replace real CCTP contracts with mocks
        ...
        uint256 balanceAfter = crossChainRemoteStrategy.checkBalance(HyperEVM.USDC);
        assertApproxEqAbs(
            balanceAfter, balanceBefore - withdrawalAmount, 1e6, "checkBalance should decrease by withdrawal amount"
        );
        ...
        bytes32 tokensBridgedTopic = keccak256("TokensBridged(uint32,address,address,uint256,uint256,uint32,bytes)");

        bool foundMessage = false;
        for (uint256 i = 0; i < entries.length; i++) {
            if (entries[i].topics[0] == messageTransmittedTopic || entries[i].topics[0] == tokensBridgedTopic) {
                foundMessage = true;
                break;
            }
        }
