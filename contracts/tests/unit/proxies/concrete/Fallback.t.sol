...
contract Unit_Concrete_Proxy_Fallback_Test is Unit_Proxies_Shared_Test {
    ...
    function test_fallback_delegatesSetValue() public {
        ...
        (bool success,) = address(proxy).call(abi.encodeWithSelector(MockImplementation.setValue.selector, 123));
        assertTrue(success);
    ...
    function test_fallback_returnsData() public {
        // Set a value first
        (bool s1,) = address(proxy).call(abi.encodeWithSelector(MockImplementation.setValue.selector, 999));
        assertTrue(s1);

        // Read it back — tests that return data is forwarded
        (bool s2, bytes memory result) =
            address(proxy).staticcall(abi.encodeWithSelector(MockImplementation.getValue.selector));
        assertTrue(s2);
    ...
    function test_fallback_revertsWhenDelegatecallReverts() public {
        (bool success, bytes memory returnData) =
            address(proxy).call(abi.encodeWithSelector(MockImplementation.revertingFunction.selector));
        assertFalse(success);
    ...
    function test_fallback_multipleCallsPreserveState() public {
        // Set value to 10
        (bool s1,) = address(proxy).call(abi.encodeWithSelector(MockImplementation.setValue.selector, 10));
        assertTrue(s1);

        // Set value to 20
        (bool s2,) = address(proxy).call(abi.encodeWithSelector(MockImplementation.setValue.selector, 20));
        assertTrue(s2);

        // Read — should be 20
        (bool s3, bytes memory result) =
            address(proxy).staticcall(abi.encodeWithSelector(MockImplementation.getValue.selector));
        assertTrue(s3);
