...
contract Unit_Concrete_SetXOGNRewardRateModule_SetBounds_Test is Unit_SetXOGNRewardRateModule_Shared_Test {
    function test_setBounds_updatesAll() public {
        ...
        module.setBounds(0.5e18, 4e18, 1000, 1 days, 3 days);
        ...
        assertEq(uint256(module.stepPeriod()), 3 days);
    }

    function test_setBounds_emitsEvent() public {
        vm.expectEmit({emitter: address(module)});
        emit ISetXOGNRewardRateModule.BoundsSet(0.5e18, 4e18, 1000, 1 days, 3 days);
    ...
    function test_setBounds_allowsEqualMinAndMax() public {
        vm.prank(address(mockSafe));
        module.setBounds(1e18, 1e18, 1e4, 1 hours, 3 days);
    ...
    function test_setBounds_RevertWhen_minAboveMax() public {
        ...
        module.setBounds(4e18, 1e18, 1000, 1 days, 3 days);
    }

    function test_setBounds_RevertWhen_zeroStep() public {
        vm.prank(address(mockSafe));
        vm.expectRevert("Invalid step");
        module.setBounds(0.5e18, 4e18, 0, 1 days, 3 days);
    }

    function test_setBounds_RevertWhen_stepAboveHundredPercent() public {
        vm.prank(address(mockSafe));
        vm.expectRevert("Invalid step");
        module.setBounds(0.5e18, 4e18, 1e4 + 1, 1 days, 3 days);
    }

    function test_setBounds_RevertWhen_zeroRunway() public {
        vm.prank(address(mockSafe));
        vm.expectRevert("Invalid runway");
        module.setBounds(0.5e18, 4e18, 1000, 0, 3 days);
    }

    /// @dev A zero period would refresh the checkpoint on every call, which is
    ///      the ratcheting behaviour the checkpoint exists to remove.
    function test_setBounds_RevertWhen_zeroStepPeriod() public {
        vm.prank(address(mockSafe));
        vm.expectRevert("Invalid step period");
        module.setBounds(0.5e18, 4e18, 1000, 1 days, 0);
    }
