...
const { isMainnet, isRinkeby, isGanacheFork, proposeArgs } = require("../../test/helpers.js");
...
function getFunctionsAbi(contract) {
  return '[' + Object.values(contract.interface.functions).map(f => f.format("json")).join(',') + ']';
}
...
async function main() {
  ...
  const tokenG = await ethers.getContractAt("Governable", (await ethers.getContract("OUSDProxy") ).address);
  const strategyG = await ethers.getContractAt("Governable", (await ethers.getContract("CompoundStrategyProxy") ).address);
  const minuteTimelock = await ethers.getContract("MinuteTimelock");
  const vaultCore =  await ethers.getContract("VaultCore");
  const pVaultCore = await ethers.getContractAt("VaultCore", vaultProxy.address);
  const pVaultAdmin = await ethers.getContractAt("VaultAdmin", vaultProxy.address);
  ...
  console.log("Make multisig call:");
  console.log(`    proposeAndQueue(${JSON.stringify(targets)}, ${JSON.stringify(values)}, ${JSON.stringify(sigs)}, ${JSON.stringify(datas)}, ${JSON.stringify(description)})`);

  if (!isMainnet) {
    console.log("We are not mainnet and so running agains governor directly...");
    ...
    await strategyG.connect(sGovernor).transferGovernance(minuteTimelock.address);

    await governor.connect(sGovernor).proposeAndQueue(targets, values, sigs, datas, description);
