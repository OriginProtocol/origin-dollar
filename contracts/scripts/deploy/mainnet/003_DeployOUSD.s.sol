...
contract $003_DeployOUSD is AbstractDeployScript("003_DeployOUSD") {
    ...
    function _execute() internal override {
        ...
        ousdProxy.initialize(
            address(ousdImpl),
            GOVERNOR,
            abi.encodeCall(OUSD.initialize, (address(vaultProxy), INITIAL_CREDITS_PER_TOKEN))
        );
        vaultProxy.initialize(address(vaultImpl), GOVERNOR, abi.encodeCall(IVault.initialize, (address(ousdProxy))));

        WrappedOUSDProxy wrappedOusdProxy = new WrappedOUSDProxy();
        WrappedOusd wrappedOusdImpl = new WrappedOusd(ERC20(address(ousdProxy)));
        wrappedOusdProxy.initialize(address(wrappedOusdImpl), GOVERNOR, abi.encodeCall(IWOToken.initialize, ()));
    ...
    function _buildGovernanceProposal() internal override {
        ...
        govProposal.setDescription("Configure and activate the fresh OUSD Vault");
        govProposal.action(vaultProxy, "setStrategistAddr(address)", abi.encode(STRATEGIST));
        govProposal.action(vaultProxy, "setRebaseRateMax(uint256)", abi.encode(REBASE_RATE_MAX));
        govProposal.action(vaultProxy, "setWithdrawalClaimDelay(uint256)", abi.encode(WITHDRAWAL_CLAIM_DELAY));
    ...
    function _fork() internal override {
        ...
        InitializeGovernedUpgradeabilityProxy ousdProxy = InitializeGovernedUpgradeabilityProxy(payable(ousdProxyAddr));
        InitializeGovernedUpgradeabilityProxy vaultProxy =
            InitializeGovernedUpgradeabilityProxy(payable(vaultProxyAddr));
        InitializeGovernedUpgradeabilityProxy wrappedOusdProxy =
            InitializeGovernedUpgradeabilityProxy(payable(wrappedOusdProxyAddr));

        require(ousdProxy.implementation() == resolver.resolve("OUSD_IMPL"), "Unexpected OUSD implementation");
        require(
            vaultProxy.implementation() == resolver.resolve("OUSD_VAULT_IMPL"), "Unexpected OUSD Vault implementation"
        );
        require(
            wrappedOusdProxy.implementation() == resolver.resolve("WRAPPED_OUSD_IMPL"),
            "Unexpected Wrapped OUSD implementation"
        );
        require(ousdProxy.governor() == GOVERNOR, "Unexpected OUSD governor");
        require(vaultProxy.governor() == GOVERNOR, "Unexpected OUSD Vault governor");
        require(wrappedOusdProxy.governor() == GOVERNOR, "Unexpected Wrapped OUSD governor");

        OUSD ousd = OUSD(ousdProxyAddr);
        IVault vault = IVault(vaultProxyAddr);
        WrappedOusd wrappedOusd = WrappedOusd(wrappedOusdProxyAddr);
        VaultValueChecker vaultValueChecker = VaultValueChecker(resolver.resolve("OUSD_VAULT_VALUE_CHECKER"));

        require(ousd.vaultAddress() == vaultProxyAddr, "OUSD Vault link is incorrect");
        require(address(vault.oToken()) == ousdProxyAddr, "Vault OUSD link is incorrect");
        require(
            ousd.rebasingCreditsPerTokenHighres() == INITIAL_CREDITS_PER_TOKEN, "Unexpected OUSD credits resolution"
        );
        ...
        require(vault.strategistAddr() == STRATEGIST, "Unexpected OUSD Vault strategist");
        require(!vault.capitalPaused(), "OUSD Vault capital is paused");
        require(vault.withdrawalClaimDelay() == WITHDRAWAL_CLAIM_DELAY, "Unexpected withdrawal claim delay");
        require(vault.rebasePerSecondMax() == REBASE_RATE_MAX / 100 / 365 days, "Unexpected maximum rebase rate");

        require(wrappedOusd.asset() == ousdProxyAddr, "Unexpected Wrapped OUSD asset");
        require(wrappedOusd.adjuster() == INITIAL_CREDITS_PER_TOKEN, "Wrapped OUSD is not initialized");
        require(address(vaultValueChecker.vault()) == vaultProxyAddr, "VaultValueChecker Vault link is incorrect");
        require(address(vaultValueChecker.ousd()) == ousdProxyAddr, "VaultValueChecker OUSD link is incorrect");
    }
}
