...
async function verify(expectedAccounts) {
  ...
  for (const account of expectedAccounts) {
    ...
    console.log(`${icon} ${i}/${expectedAccounts.length} ${account.address} ${totalText}`)
    if (accountIsCorrect) {
  ...
  return {
    isCorrect, actualTotal, incorrect, correct
  }
}
...
async function uploadAccounts(accounts) {
  ...
  console.log(`Uploading batch of ${addresses.length} accounts. Total: ${batchTotal}`)
