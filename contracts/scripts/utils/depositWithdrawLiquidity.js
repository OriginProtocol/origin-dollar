...
async function main() {
  ...
  const usdt = await ethers.getContractAt(USDTAbiContainer.abi, addresses.mainnet.USDT);
  ...
  uniswapPairOUSD_USDT = await ethers.getContractAt(ERC20Abi, 
  ...
  const uniswapRouter = await ethers.getContractAt("IUniswapV2Router", addresses.mainnet.uniswapRouter);
  ...
  if (components.length == 2) {
    ...
    if (components[1].length > 6) {
      ousdBalance = ousdBalance.substr(0, ousdBalance.length - (components[1].length - 6));
    }
  }
  ...
  await ousd.connect(signers[0]).approve(uniswapRouter.address, ousdDesired, txOpts);
  console.log("Usdt approval", usdt.address);
  await usdt.connect(signers[0]).approve(uniswapRouter.address, 0, txOpts);
  await usdt.connect(signers[0]).approve(uniswapRouter.address, usdtDesired, txOpts);
  console.log("Usdt approved");

  console.log("Pair address:", uniswapPairOUSD_USDT.address);
  console.log("[pre ]Pair balance is:", utils.formatUnits(await uniswapPairOUSD_USDT.balanceOf(signerAddress), 18));

  if (ousdDesired.gt(0)) {
    await uniswapRouter.connect(signers[0]).addLiquidity(ousd.address, usdt.address, 
      ousdDesired, usdtDesired,
      0, 0, 
      signerAddress,  Date.now() + 30, txOpts); // give it 30 seconds
  }
  ...
  console.log("[addL]Pair balance is:", utils.formatUnits(await uniswapPairOUSD_USDT.balanceOf(signerAddress), 18));

  const rewardRate = await liquidityContract.rewardPerBlock();

  console.log("Reward rate for liquidity is:", utils.formatUnits(await liquidityContract.rewardPerBlock(), 18));

  await uniswapPairOUSD_USDT.connect(signers[0]).approve(liquidityContract.address, liquidityBalance);
  console.log("Depositing...");
  await liquidityContract.connect(signers[0]).deposit(liquidityBalance, txOpts);
  console.log("[depo]Pair balance is:", utils.formatUnits(await uniswapPairOUSD_USDT.balanceOf(signerAddress), 18));

  console.log("expected rewards:", utils.formatUnits(await liquidityContract.pendingRewards(signerAddress), 18));
  console.log("balance of ogn:", utils.formatUnits(await ogn.balanceOf(signerAddress), 18));
  ...
  console.log("[exit]Pair balance is:", utils.formatUnits(await uniswapPairOUSD_USDT.balanceOf(signerAddress), 18));
  console.log("exit rewards:", utils.formatUnits(await ogn.balanceOf(signerAddress), 18));
