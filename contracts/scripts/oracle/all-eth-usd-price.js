...
async function main() {
  // Get contracts.
  const uniswapOracle = await ethers.getContractAt("IViewEthUsdOracle", uniswapOracleAddress)
  const chainlinkOracle = await ethers.getContractAt("IViewEthUsdOracle", chainlinkOracleAddress)
  ...
  console.log("Uniswap   eth price:", (await uniswapOracle.ethUsdPrice()).toString());
  console.log("Chainlink eth price:", (await chainlinkOracle.ethUsdPrice()).toString());
