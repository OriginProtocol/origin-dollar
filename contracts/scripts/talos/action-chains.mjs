...
const ALL_CHAINS = [1, 8453, 146, 560048, 999, 42161, 98866];
const NAME = {
  1: "eth",
  8453: "base",
  146: "sonic",
...
function readActionMeta(file) {
  ...
  const imports = [...txt.matchAll(impRe)]
    .map((m) => m[1])
...
const actions = readdirSync(ACT_DIR)
  .filter((f) => f.endsWith(".ts"))
  .map((f) => readActionMeta(join(ACT_DIR, f)));
...
for (const [key, names] of [...byChainSet.entries()].sort(
...
for (const [u, set] of [...utilChains.entries()].sort(
