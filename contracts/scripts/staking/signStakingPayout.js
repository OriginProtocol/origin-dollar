...
const TEST_STAKING_PK = "0x345c8d05224b66bab10e9f952dc1d332e59e062be5990f87206a67e4545e132d";
...
async function signForStaking(type, address, duration, rate, amount) {
  ...
  const message = utils.solidityPack(['uint8', 'address', 'uint', 'uint', 'uint'], [type, address, duration, rate, amount]);
...
async function signPayouts(payoutList) {
  ...
  for (const p of payouts) {
    ...
    o[address] = {type, duration, 
      rate: solRate.toString(),
      amount: solAmount.toString(), 
      signature: await signForStaking(type, address, duration, solRate, solAmount)};
  }
  return o;
}
...
async function main() {
  if (process.argv.length < 4) {
    console.log(`Usage: node signStakingPayout.js <inputJSONFile> <outputJSONFile>`);
  }
